...
class HeatConsumer(BranchWOInternalsComponent):
    ...
    def create_pit_branch_entries(cls, net, branch_pit):
        ...
        hc_pit[~np.isnan(qext), IdxBranch.QEXT] = qext[~np.isnan(qext)]
        mdot = net[cls.table_name()].controlled_mdot_kg_per_s.values
        hc_pit[~np.isnan(mdot), IdxBranch.MDOTINIT] = mdot[~np.isnan(mdot)]
        treturn = net[cls.table_name()].treturn_k.values
        mask_tr = ~np.isnan(treturn)
        hc_pit[mask_tr, IdxBranch.TOUTINIT] = treturn[mask_tr]
        hc_pit[:, IdxBranch.FLOW_RETURN_CONNECT] = True
    ...
    def adaption_before_derivatives_hydraulic(cls, net,
        ...
        if np.any(mask):
            ...
            mass = hc_pit[mask, IdxBranch.QEXT] / (cp * deltat)
    ...
    def adaption_after_derivatives_hydraulic(cls, net,
        ...
        hc_pit = branch_pit[f:t, :]
        hc_pit[:, IdxBranch.DF_DP_F_B] = 0
        hc_pit[:, IdxBranch.DF_DP_T_B] = 0
        hc_pit[:, IdxBranch.DF_DM_B] = 1
        hc_pit[:, IdxBranch.F_B] = 0
        ...
        if np.any(mask):
            ...
            t_out = hc_pit[:, IdxBranch.TOUTINIT]

            df_dm = - cp * (t_out - t_in)
            mask_equal = t_out >= t_in
            mask_zero = hc_pit[:, IdxBranch.QEXT] == 0
            mask_ign = mask_equal | mask_zero
            hc_pit[mask & mask_ign, IdxBranch.MDOTINIT] = 0
            hc_pit[mask & ~mask_ign, IdxBranch.DF_DM_B] = df_dm[mask & ~mask_ign]
    ...
    def adaption_before_derivatives_thermal(cls, net,
        ...
        if np.any(mask):
            cp = get_branch_cp(get_fluid(net), node_pit, hc_pit)
            q_ext = cp[mask] * hc_pit[mask, IdxBranch.MDOTINIT] * consumer_array[mask, cls.DELTAT]
        ...
        if np.any(mask):
            ...
            q_ext = cp[mask] * hc_pit[mask, IdxBranch.MDOTINIT] * (t_in - t_out)
            hc_pit[mask, IdxBranch.QEXT] = q_ext
    ...
    def adaption_after_derivatives_thermal(cls, net,
        ...
        mask= consumer_array[:, cls.MODE] == cls.QE_TR
        if np.any(mask):
            mask_ign = hc_pit[:, IdxBranch.QEXT] == 0
            mask = mask & ~mask_ign
            hc_pit[mask, IdxBranch.LOAD_VEC_B_T] = 0
            hc_pit[mask, IdxBranch.JAC_DERIV_DTOUT] = 1
            hc_pit[mask, IdxBranch.JAC_DERIV_DT] = 0
