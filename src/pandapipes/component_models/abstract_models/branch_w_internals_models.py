...
class BranchWInternalsComponent(BranchComponent):
    ...
    def create_pit_branch_entries(cls, net, branch_pit):
        ...
        if not get_net_option(net, "transient") or get_net_option(net, "simulation_time_step") == 0:
            ...
            set_entry_check_repeat(branch_w_internals_pit, IdxBranch.D, net[tbl].inner_diameter_mm.values / 1000., internal_branch_number,
                has_internals)
            if "outer_diameter_mm" in net[tbl]:
                set_entry_check_repeat(branch_w_internals_pit, IdxBranch.DO, net[tbl].outer_diameter_mm.values / 1000., internal_branch_number,
                    has_internals)
                branch_w_internals_pit[np.isnan(branch_w_internals_pit[:, IdxBranch.DO]), IdxBranch.DO] = (
                    branch_w_internals_pit)[np.isnan(branch_w_internals_pit[:, IdxBranch.DO]), IdxBranch.D]
            else:
                set_entry_check_repeat(branch_w_internals_pit, IdxBranch.DO, net[tbl].inner_diameter_mm.values / 1000., internal_branch_number,
                    has_internals)
            ...
            branch_w_internals_pit[:, IdxBranch.QEXT] = 0.0
        return branch_w_internals_pit, node_pit
