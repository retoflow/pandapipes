...
def calculate_derivatives_hydraulic(net,
    ...
    der_lambda = calc_der_lambda(branch_pit[:, IdxBranch.MDOTINIT], eta,
                                 ...
                                 re, branch_pit[:, IdxBranch.LENGTH])
...
def calculate_derivatives_thermal(net,
    ...
    node_pit[:, IdxNode.LOAD_T] = fn
    node_pit[:, IdxNode.JAC_DERIV_DT_N] = dfn_dt

    branch_pit[:, IdxBranch.LOAD_VEC_BRANCHES_T] = fb
    branch_pit[:, IdxBranch.JAC_DERIV_DT_NODE] = dfb_dt
    branch_pit[:, IdxBranch.JAC_DERIV_DTOUT_NODE] = dfb_dtout

    branch_pit[:, IdxBranch.F1_T_N] = fnt
    branch_pit[:, IdxBranch.JAC_DERIV_DT_NODE] = dfnt_dt
    branch_pit[:, IdxBranch.JAC_DERIV_DTOUT_NODE] = dfnt_dtout
